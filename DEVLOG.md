...
conversion had retained `nullable: true` but removed `null` from the
`ailocals_presence` capability `reason` enum. The protocol and busy-worker
fixture permit `reason: null`, so the enum now retains its unquoted YAML
`null` member. `type: string`, `nullable: true`, the seven string reasons,
and the OpenAPI 3.0.3 version remain unchanged. No provider or worker code
changed, and CP2/live acceptance remains untouched.

Extended `web/scripts/validate-openapi.mjs` with a focused regression check:
null and every supported string reason are accepted, an unknown reason is
rejected, and an in-memory enum with null removed fails the negative control.
API output was regenerated only through the existing generator; no manual
generated-file edit was made. All implementation changes remain uncommitted
for reviewer approval; no public push or deployment was performed.

### Verification

- `npm --prefix web run validate:openapi`: passed, including the nullable
  reason assertions and CP12 matrix.
- `npm --prefix web run generate:api` twice with an isolated scratch copy:
  byte-identical; no additional generated diff beyond the existing CP1
  contract consequence.
- `npm --prefix web run check`: 0 errors, 0 warnings.
- `git diff --check`: clean.
- Final status/diff scope preserves `.github/workflows/deploy.yml`,
  `DEVLOG.md`, `Makefile`, `contracts/openapi.yaml`,
  `web/scripts/validate-openapi.mjs`, `web/src/lib/api/generated.ts`, and
  `web/tests/e2e/settings-experiments.spec.ts`. Previously passing full Go
  and browser suites were not repeated for this schema/assertion-only fix.

## 2026-09-09 — Checkpoint 1 recovery: deterministic CP14 verification

Implemented and verified Checkpoint 1 of
`plans/in-progress/doublangu-finish-reader-settings-20260909.md`, the
owner-authorized recovery of pending original Checkpoint 14. The four known
dirty CP14 files were preserved, along with approved Checkpoints 1–13 at
`9f3ea83`; no historical CP6/CP7 patches were reapplied and no failed-run
artifacts were rewritten. The recovery is related to failed run
`20260909t080503z-63ce69b4`; its artifacts remain historical evidence.
All changes remain uncommitted for review; no checkpoint commit was created.

### Implementation

- `web/tests/e2e/settings-experiments.spec.ts` (new): the last missing
  file from the section-8 E2E list. Three mocked-API acceptance tests on
  the real Settings page: saving a prompt version reports "Saved v3 for
  Explore. Pin it on a profile to use it; nothing was activated" and
  issues zero profile writes; Alpha pins Explore v2 while Beta keeps v1
  with exactly one PUT; Explore model `model-b` saves independently while
  the Translation binding stays `model-a`.
- `Makefile`: new `test-reader-settings` target (`internal/prompts`,
  `internal/pipeline`, `internal/analysis`, `internal/sentencetranslation`,
  `internal/reader`) wired into `verify` and declared phony, closing the
  section-9 gap between the plan's package list and the CI gate.
- `.github/workflows/deploy.yml`: the reader E2E step now runs the full
  section-8 suite list, adding `settings-experiments`, `reader-condensed`,
  `sentence-translation`, and `navigation-menu` to the previous five.
- `contracts/openapi.yaml`: replaced the OpenAPI 3.1-only `const` and
  multi-type nullable declaration in the `ailocals.v1` presence contract
  with OpenAPI 3.0-compatible `enum`/`nullable` forms, and documented the
  existing dictionary-entry `400` response required by the approved CP12
  semantic matrix. Wire types and required fields are unchanged.
- `web/src/lib/api/generated.ts`: regenerated by `generate:api`; the only
  generated changes are the documented dictionary-entry response and the
  enum annotation. No generated file was edited manually.
- The preceding worker's `/tmp/cp14-rollback` scratch rehearsal remains
  outside the repository and is not claimed as Checkpoint 1 evidence. It
  used a fresh current-schema database; Checkpoint 2 must still prove
  rollback from a populated pre-upgrade backup.

### Verification

- `go test` section-8 package list (prompts/pipeline/analysis/annotator/
  dictionary/sentencetranslation/reader/jobs/store/httpapi/server): all ok.
- `go test -race` section-8 list (prompts/analysis/dictionary/
  sentencetranslation/jobs/semantics/httpapi): all ok.
- `npm --prefix web run validate:openapi`: passed, including the CP12
  semantic matrix.
- `npm --prefix web run generate:api` twice, byte-identical (`cmp` clean);
  generated output changed only as a consequence of the contract fix.
- `npm --prefix web run check`: 0 errors, 0 warnings.
- `npm --prefix web run test:unit -- src/lib/settings src/lib/reader
  src/lib/routes`: 25 files, 133 tests passed.
- `npm --prefix web run build`: static site written.
- `make verify` (with the new `test-reader-settings` target): exit 0;
  its complete web unit gate reported 29 files and 185 tests passed.
- `npm --prefix web run test:e2e` full section-8 list (9 files, 50 tests):
  all passed, including the 3 new settings-experiments tests.
- `npm exec -- prettier --check tests/e2e/settings-experiments.spec.ts`
  (from `web/`): passed.
- `git diff --check`: clean.
- Authenticated live tests, the real owner UI path, and populated rollback
  proof were not run in this checkpoint; they remain Checkpoint 2 work and
  are not claimed as passes. No delivery or public push was performed.

### Delivery state

- Tested revision: `9f3ea83` plus the uncommitted scoped CP14 recovery
  changes. Checkpoints 1–13 are approved prior history; this entry records
  deterministic Checkpoint 1 evidence only. Exact file scope is available
  via `git status --short`.
- Checkpoint 2 remains for approved nonproduction catalog/provider access,
  real owner-browser generation evidence, populated rollback rehearsal, and
  final readiness records. Public deployment approval remains separate; a
  main push triggers deployment.

## 2026-09-09 — Checkpoint 13: Condensed article mode

Implemented Checkpoint 13 of
`plans/in-progress/reader-settings-prompt-experiments-20260908.md` on top
of the uncommitted Checkpoint 12 work, which was preserved untouched.
Only the reading-mode helper, ReaderToolbar switch, ArticleReader mode
state plus condensed action row, Paragraph/Sentence/TextOccurrence
condensed presentation, the new `reader-condensed.spec.ts`, and reader
docs changed. No backend, contract, generated-client, or speech behavior
changed. Verified work is left uncommitted for review; no checkpoint
commits were created.

### Implementation

- `readingMode.ts` (new): `learning|condensed` with Learning the default,
  persisted per browser under `doublangu.reader.readingMode.v1`. Invalid,
  missing, or blocked storage falls back to learning without throwing.
  The server-owned pronounce-on-hover preference is untouched.
- `ReaderToolbar.svelte`: Learning/Condensed switch reusing the existing
  toolbar chrome (same buttons, no new palette). Defaults keep the
  previous props working.
- `ArticleReader.svelte`: mode state restored on mount and saved on
  change; `data-reading-mode` on the shell; focus scaling neutralized in
  Condensed (`--reader-rest-scale: 1`). One fixed-height action row under
  the toolbar (always the same `min-height`, so enabling buttons never
  adds gaps) with Play sentence and Translate, both disabled without an
  active sentence and a "No active sentence" label. The row's Translate
  mirrors the footer control: 350ms dwell for hover/focus, immediate for
  touch/click, with the same focus-return guard. Its popover is keyed to
  the active sentence; focusing another sentence, switching modes, or
  navigating closes it so no stale text mislabels.
- `Paragraph`/`Sentence`/`TextOccurrence`: same components render both
  modes via a `condensed` prop (no second renderer). Condensed flows
  sentence wrappers inline with source whitespace preserved, keeps the
  responsive article size at 1.65 line-height, renders no subtitles row
  (spans stay in the DOM with `display:none`), no construction overlay,
  no per-sentence footer, and no focus scaling. Word hover/click and
  source punctuation are unchanged.
- Tests: `readingMode.test.ts` (8: defaults, invalid values, blocked
  storage) and `ArticleReaderReadingMode.test.ts` (2: default learning
  without the row, stored-condensed restore), plus
  `web/tests/e2e/reader-condensed.spec.ts` (8: default/decoration hiding
  with exact source fidelity, reload persistence with invalid-storage
  fallback, focus stability at 320/375/1280px, action-row tracking with
  identical bounding boxes, word hover/click, return to learning).
- `web/src/lib/reader/AGENTS.md`: subtitle/geometry invariants now apply
  to Learning mode; Condensed hiding, action-row ownership, storage key,
  and the condensed E2E suite are documented.

### Verification

- `npm --prefix web run test:unit -- src/lib/reader`: 68 passed (15 files).
- `npm --prefix web run check`: 0 errors, 0 warnings.
- `npm --prefix web run test:e2e -- reader-condensed.spec.ts`: 8 passed.
- `npm --prefix web run test:e2e -- reader-design.spec.ts`: 5 passed (no regressions).
- `npm --prefix web run test:e2e -- reader-preference.spec.ts`: 4 passed (no regressions).
- `npm --prefix web run test:e2e -- sentence-translation.spec.ts`: 7 passed (no regressions).
- Observed: mode persists across articles/reload; condensed paragraphs
  carry the exact fixture source; focusing middle/end sentences at
  320/375/1280px leaves every paragraph/word box and the scroll offset
  identical with no horizontal overflow; the action row keeps one
  bounding box from "No active sentence" through focus to Escape;
  learning-mode subtitles, footers, and tools return on toggle back.

## 2026-09-09 — Checkpoint 12: Sentence and Explore controls

Implemented Checkpoint 12 of
`plans/in-progress/reader-settings-prompt-experiments-20260908.md` on top
of the uncommitted Checkpoint 11 work, which was preserved untouched.
Only the reader sentence/Explore controls, their controllers and tests,
the new `sentence-translation.spec.ts`, and reader docs changed. No
backend, contract, or generated-client behavior changed. Verified work is
left uncommitted for review; no checkpoint commits were created.

### Implementation

- `sentenceTranslationController.ts` (new): per-sentence durable state
  machine mirroring the Explore controller. `open` is a read-only lookup;
  `ensure` POSTs `{mode:ensure}` only while still missing (ready, pending,
  and failed subjects are left alone, so hovers never loop a failure);
  `regenerate` always POSTs `{mode:regenerate}` and keeps the old text
  visible; duplicate calls while pending rejoin the same work via a
  module-level in-flight map keyed by article+sentence+mode. Polling uses
  the read-only GET, stops on close/subject change, and discards stale
  responses by generation identity.
- `SentenceTranslationPopover.svelte` (new): dialog with the existing
  popover conventions (viewport clamping, mobile sheet, Escape/outside
  close, focus return). Shows saved text with Regenerate, retained text
  with replacing progress, failure with Retry (initial) or Retry
  regeneration (replacement) plus a View-run link when a run exists, and
  "No active sentence" without a subject. Translation renders only for the
  matching sentence id.
- `Sentence.svelte`: Translate button (`⇄ Translate`, usable without
  audio) next to Play sentence. Hover/focus opens the popover and sends
  ensure after a 350ms dwell; touch/click sends it immediately; leaving
  before the dwell cancels without any POST. Closing returns focus to the
  trigger with a guard so the focus event cannot reopen the popover.
- `exploreController.ts` / `ExplorePanel.svelte`: `regenerate` POSTs
  `{retry:false, regenerate:true}`, retains the saved document through
  queued/generating/failed states, carries `run_id`, and rejoins pending
  work instead of reposting. The panel shows Regenerate on saved entries,
  the old entry with inline progress while replacing, and Retry
  regeneration plus View run on replacement failure.
- Tests: `sentenceTranslationController.test.ts` (9), new popover tests
  (6), Explore controller regenerate tests (+3) and panel tests (+2), and
  `web/tests/e2e/sentence-translation.spec.ts` (7: dwell generation,
  quick-leave silence, saved-hover silence, failed Retry/View-run without
  loops, failed-regeneration retention, cross-sentence labeling, Explore
  regenerate retention with duplicate guard).
- `web/src/lib/reader/AGENTS.md`: documents the Translate dwell/polling/
  retry/run-link behavior and the Explore regenerate retention/guard
  behavior.

### Verification

- `npm --prefix web run test:unit -- src/lib/reader`: 58 passed (13 files).
- `npm --prefix web run check`: 0 errors, 0 warnings.
- `npm --prefix web run test:e2e -- sentence-translation.spec.ts`: 7 passed.
- `npm --prefix web run test:e2e -- reader-explore.spec.ts`: 4 passed (no regressions).
- Observed: missing hover generates once after the dwell; quick leave
  sends nothing; saved hover never calls the LLM; failed hovers never
  retry while explicit Retry/Regenerate work and keep old text; opening a
  second sentence closes the first popover so no stale text mislabels;
  Explore regeneration keeps the old entry with progress and one POST.

## 2026-09-09 — Checkpoint 11: Analysis runs for all operations

Implemented Checkpoint 11 of
`plans/in-progress/reader-settings-prompt-experiments-20260908.md` on top
of the uncommitted Checkpoint 10 work. Only the run list/detail history
surface, the `operation` list filter, the OpenAPI runs contract plus
regenerated client, and the analysis-runs routes changed. No lifecycle,
scheduler, or prompt-execution behavior changed: the run phase lifecycle
stays running-to-finished as built in checkpoint 6 (nothing writes the
`queued` phase yet), and the UI renders Queued distinctly when that phase
is present. Verified work is left uncommitted for review; no checkpoint
commits were created.

### Implementation

- `internal/analysis/history.go`: `Run`/`RunSummary` now carry
  `operation_type`, `subject_id`, `subject_label`, and `phase`;
  `StageAttemptSummary` now carries `error_phase`. New
  `ListRunsFiltered` validates the operation against the stored CHECK
  values (`article_analysis`/`explore`/`sentence_translation`, empty for
  all) and returns `ErrInvalidRunQuery` otherwise; `ListRuns` delegates
  unfiltered so existing callers keep legacy behavior.
- `internal/httpapi/analysis.go`: `ServeRuns` passes the `operation`
  query parameter through; unknown values return 400 via the existing
  validation error shape.
- `contracts/openapi.yaml`: `operation` query parameter on
  `GET /api/v1/analysis/runs`; optional `operation_type`/`subject_id`/
  `subject_label`/`phase` on `AnalysisRunSummary`/`AnalysisRun`;
  optional `error_phase` on `AnalysisStageAttempt`; optional
  `prompt_snapshots` on `AnalysisProfileSnapshot` with a new
  `AnalysisPromptSnapshot` schema (type/id/version/content_hash/
  envelope_version plus instruction text). Fields stay optional so
  legacy rows without snapshots keep working.
- `web/src/lib/api/client.ts`: `listAnalysisRuns` accepts `operation`
  and only sends it when set, preserving the existing request shape.
  `generated.ts` regenerated twice byte-identical.
- List route: All / Article analysis / Explore / Sentence translation
  filter that reloads from the first page; an Operation column with the
  subject label/id; Queued status for running runs in the queued phase.
  Legacy rows without an operation read as Article analysis.
- Detail route: operation eyebrow and summary row, subject and phase
  rows, exact generation/correction prompt versions from the stored
  snapshot (`type vN · id · hash`), per-attempt failure phase, and
  accurate zero-turn copy (cache note only on a cache hit, named
  failure otherwise, "No provider response was received." when nothing
  was captured). Failed runs with no attempts/turns/detail get the same
  plain note. Legacy list/detail behavior is retained.
- Tests: `history_operations_test.go` (filter matrix, summary/detail
  exposure, unknown-operation rejection, error-phase exposure),
  `TestAnalysisHTTPOperationFilter` (filtered/empty/400 paths), and
  route tests for the filter, Queued display, prompt versions,
  error phase, and all three zero-turn copies.

### Verification

- `go test ./internal/analysis ./internal/httpapi -count=1`: all ok.
- `npm --prefix web run test:unit -- src/lib/routes`: 23 passed (6 files).
- `npm --prefix web run check`: 0 errors, 0 warnings.
- `generate:api` twice byte-identical (`cmp` clean).
- `npm --prefix web run validate:openapi` still fails on the
  pre-existing worker/ailocals schema errors only (70 instancePath
  entries, none referencing any analysis path or schema). No new
  contract errors.
- `git diff --check` clean; `gofmt` clean on touched Go files
  (`internal/httpapi/ailocals.go` was already unformatted, untouched).
- Observed: Explore/sentence/article runs are filterable; a queued-phase
  row reads Queued; failed attempts expose the failure phase and the
  bounded actual prompt/response/settings.
...
## 2026-09-09 — Companion CP1 v02 reviewer approval

Reviewed the seven pending deterministic CP14 recovery files against `9f3ea83`
using the worktree fallback. No material findings remain; the null-enum finding
is resolved. Reviewer verification: `npm --prefix web run validate:openapi`,
`npm --prefix web run generate:api` twice with temporary-copy `cmp` checks,
`npm --prefix web run check` (zero errors/warnings), and `git diff --check` all
passed. An independent Node/SwaggerParser/Ajv probe accepted the existing busy
presence fixture, rejected an unknown reason, and detected removing null from
the enum. Earlier full deterministic suite evidence above remains applicable;
those suites were not repeated for this bounded schema/assertion correction.
