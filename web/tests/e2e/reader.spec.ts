...
test('renders the v2 source immediately, then layers subtitles and shared construction markers', async ({ page }) => {
	...
	await expect(page.locator('.article-options summary')).toContainText('Preparing English subtitles…');
	...
	await expect(page.locator('[data-connector-for="construction-give-up"] path').first()).toBeVisible();
...
test('uses sense-keyed learning, explicit hover audio, and lazy narration playback', async ({ page }) => {
	...
	await expect(page.getByRole('dialog')).toBeVisible();
	await page.getByRole('button', { name: 'Mark learned' }).click();
	await expect(page.getByText('Marked learned. Subtitle stays visible.')).toBeVisible();
	await expect(page.locator('[data-occurrence-id="token-geef"] .translation-subtitle')).toHaveText('give');
