...
// ExplorePromptSnapshots captures the profile's pinned explore generation
// and correction versions as immutable execution snapshots for one on-demand
// explore operation.
func (s *ProfileStore) ExplorePromptSnapshots(ctx context.Context, profileID string) ([]pipeline.PromptSnapshot, error) {
	...
	for _, promptType := range []prompts.PromptType{prompts.TypeExplore, prompts.TypeCorrection} {
		versionID, ok := selections[promptType]
		if !ok {
